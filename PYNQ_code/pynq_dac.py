...
class DAC3484:
    ...
    def set_dac_nco(self, channel: int, freq_mhz: float, phase_deg: float = 0) -> bool:
        ...
        print(f"\n[NCO] DAC{self.dac_num} ch{channel}: {freq_mhz} MHz, {phase_deg}°")

        self.configure_dac_default()

        # Enable NCO in CONFIG2
        reg2 = self.read_register(DAC_REG_CONFIG2)
        self.write_register(DAC_REG_CONFIG2, reg2 | 0x0050)
        ...
        print(f"✓ NCO configured: word=0x{nco_upper:04X}{nco_lower:04X}\n")
        return True
