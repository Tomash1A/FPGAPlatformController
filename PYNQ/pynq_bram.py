...
---------------------------
    Overlay alias                              Role
    ─────────────────────────────────────────────────────────────────
    MT_0/IQ_32tones_w_bram/axi_bram_ctrl_0    Frequency BRAM – DAC1 AB (IQ ch1)
    MT_1/IQ_32tones_w_bram/axi_bram_ctrl_0    Frequency BRAM – DAC1 CD (IQ ch2)
...
Frequency BRAM data format (per signal, 8 bytes = 2 × 32-bit words):
    ...
    Word 1 (higher byte address) : amplitude + phase
        ...
        bits [15: 0] = amp_word    (int(amp_0_to_1 × 65535), unsigned)
...
DDS_CLOCK_HZ: int = 250_000_000   # 62.5 MHz
...
_FREQ_MEM_NAMES: dict[int, str] = {
    1: 'MT_0/IQ_32tones_w_bram/axi_bram_ctrl_0',
    2: 'MT_1/IQ_32tones_w_bram/axi_bram_ctrl_0',
}

_AMP_MEM_NAMES: dict[int, str] = {
    1: 'MT_0/axi_bram_ctrl_0',
    2: 'MT_1/axi_bram_ctrl_0',
}
...
def _pack_signal(amp: float, freq_hz: float,
    ...
    amp_word   = max(0, min(65535, int(amp * 65535))) & 0xFFFF
...
def _unpack_signal(word0: int, word1: int) -> tuple[float, float, float]:
    ...
    raw_amp   = word1 & 0xFFFF
    amplitude = raw_amp / 65535.0
...
class BRAMController:
    ...
    def _calc_freq_offset(self, profile_num: int, signal_num: int) -> int:
        """
        Calculate byte offset for a signal matching hardware addr_control scheme:
        - 8 addresses per profile
        - 4 tone words (64-bit each) per address
        - Byte offset = (profile * 8 + signal // 4) * 32 + (signal % 4) * 8

        Example: Profile 0, Signal 0 → address 0x0000, tone 0 → byte 0x00
                 Profile 0, Signal 4 → address 0x0001, tone 0 → byte 0x20
                 Profile 1, Signal 0 → address 0x0008, tone 0 → byte 0x100
        """
        bram_addr = profile_num * 8 + (signal_num // 4)
        tone_in_addr = signal_num % 4
        byte_offset = bram_addr * 32 + tone_in_addr * 8
        return byte_offset

    def _validate_freq_data(self, profile_num: int, data: list) -> bool:
        """
        Validate frequency profile data before writing to BRAM.

        Constraints:
        - Profile: 0-3 (4 profiles from Arduino pins)
        - Signals: 0-31 (8 addresses × 4 tones per address)
        - Amplitude: 0.0-1.0
        - Frequency: 0-125 MHz (Nyquist limit for 250 MHz clock)
        - Phase: 0-360°
        """
        # Profile validation
        if not (0 <= profile_num <= 3):
            print(f"✗ Profile number {profile_num} out of range (0–3)")
            return False
        ...
        # Signal count validation (8 addresses × 4 tones per address = 32 max)
        if len(data) > 32:
            print(f"✗ Too many signals ({len(data)}); max 32 per profile")
            return False
        ...
        for idx, sig in enumerate(data):
            ...
            if len(sig) != 3:
                print(f"✗ Signal {idx}: must be (amp, freq_mhz, phase_deg), got {sig}")
                return False
            ...
            if not isinstance(amp, (int, float)):
                print(f"✗ Signal {idx}: amplitude must be numeric, got {type(amp)}")
                return False
            if not (0.0 <= amp <= 1.0):
            ...
            if not isinstance(freq_mhz, (int, float)):
                print(f"✗ Signal {idx}: frequency must be numeric, got {type(freq_mhz)}")
                return False
            nyquist_mhz = DDS_CLOCK_HZ / 2e6
            if not (0 <= freq_mhz <= nyquist_mhz):
                print(f"⚠ Signal {idx}: freq {freq_mhz} MHz exceeds Nyquist"
                      f" ({nyquist_mhz:.1f} MHz) — will produce aliasing")

            # Phase validation
            if not isinstance(phase_deg, (int, float)):
                print(f"✗ Signal {idx}: phase must be numeric, got {type(phase_deg)}")
                return False
    ...
    def set_freq_profile(self, channel: int, profile_num: int,
        ...
        for sig_num in range(len(data), _SIGNALS_PER_PROFILE):
            offset = self._calc_freq_offset(profile_num, sig_num)
            self._write_freq_word(channel, offset,     0)
            self._write_freq_word(channel, offset + 4, 0)
