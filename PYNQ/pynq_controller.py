...
class PynqController:
    ...
    def __init__(self, ol, board_ver: int = 2, enable_bram: bool = True, enable_axi: bool = False):
        """
        Args:
            ...
            enable_bram : Set False for overlays without MT_0/MT_1 BRAMs
                          (e.g. ADC-only test bitstreams).
            enable_axi  : Set True to initialize AXI subsystem for DDS (when BRAM unavailable).
                          Use in ADC-only designs that have DDS IP but no BRAM.
        """
        ...
        if enable_bram:
            ...
            self.bram = None
            print("  (BRAM controller disabled)")

        # AXI subsystem initialization for DDS (when BRAM unavailable)
        if enable_axi and not enable_bram:
            self._init_axi_subsystem()
            print("  (AXI subsystem initialized for DDS)")
    ...
    def _init_axi_subsystem(self) -> bool:
        """
        Initialize AXI interconnect for DDS IP access.
        Called when enable_axi=True and enable_bram=False.
        Ensures AXI GPIO (DDS_CTRL) is accessible for DDS configuration.
        """
        try:
            # Verify DDS_CTRL GPIO is accessible (needed by DAC2_DDS)
            gpio = self.ol.ST_0.DDS_CTRL
            print("    ✓ DDS_CTRL AXI GPIO accessible")
            return True
        except AttributeError:
            print("    ⚠ DDS_CTRL GPIO not found in overlay (ST_0/DDS_CTRL). "
                  "Verify XSA has DDS_CTRL AXI GPIO block.")
            return False
    ...
    def configure_clock(self, config_string: str, debug=0) -> bool:
        """
        ...
        """
        self.cdce.configure(config_string, debug)
        return True
