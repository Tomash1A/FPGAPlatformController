@echo off
REM -----------------------------------------------------------------------
REM  build.bat  —  Creates the REV Vivado project
REM  Block Design: DAC1ST125MHz_DAC2MT125MHz_wILA
...
echo Building REV project (DAC1ST125MHz_DAC2MT125MHz_wILA)...
vivado -mode batch -source "%SCRIPT_DIR%build.tcl" -tclargs "%SCRIPT_DIR%"

echo.
echo Done. Open REV\REV.xpr in Vivado, then click Generate Bitstream.
pause
